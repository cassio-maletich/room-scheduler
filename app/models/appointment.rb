class Appointment < ApplicationRecord
  ...
  def check_conflict
    if Appointment.where(room_id: self.room_id).where('(start >= :st AND start < :end) OR ("appointments"."start" <= :st AND "appointments"."end" > :st)', { st: self.start.to_datetime, end: self.end.to_datetime }).present?
